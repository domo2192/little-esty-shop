...
RSpec.describe "As an admin" do
	...
	it "should show the total revenue for admin invoices with discounts " do
		...
		visit "/admin/invoices/#{invoice1.id}"
		expect(page).to have_content("Invoice Total After Discount:$0.71")
	end

	it "it shows the correct amount for discounts " do
		InvoiceItem.destroy_all
		Transaction.destroy_all
		Item.destroy_all
		Invoice.destroy_all
		...
		transaction1 = create(:transaction, invoice_id: invoice1.id, result: "success")
		visit "/admin/invoices/#{invoice1.id}"
		expect(page).to have_content("Invoice Total After Discount:$0.64")
	end
end
